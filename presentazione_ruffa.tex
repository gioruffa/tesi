...
\column{0.5\textwidth}
	\only<1>{
	\includegraphics[width=1.1\textwidth, height=0.55 \textheight]{beam_threads_MPIonly.pdf}	
	}
	\only<2>{
	\includegraphics[width=1.1\textwidth, height=0.55 \textheight]{beam_threads_MPIonly_cdiaghg.pdf}	
	}
	%\only<3>{
...
\column{0.5\textwidth}
	...
	\begin{block}{Comportamento}
		\begin{itemize}
			\visible<1->{\item MPI: parallelizzazione multiprocesso a memoria distribuita}
			\visible<2->{\item Diagonalizzazione ScaLapack inefficiente}
			\visible<3->{
			\item Threads forzano diagonalizzazione ``\textit{seriale}"
			\item MKL con threads pi\`u efficiente
			}
		\end{itemize}
	\end{block}
	}
