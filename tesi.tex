...
For a diagonal set of $\{\psi^{KS}_{j}\}$\footnote{Guaranteed to exist after the diagonalization of \eqref{eq:DFTH}} equations the expectation value of the total Energy has the form :
\begin{align}
...
As a final consideration of this theoretical section we must say that from a physical standpoint the Kone-Shan eigenvalues $\varepsilon^{KS}_j$ do not really represents energies. 
Also the orbitals  $\{\psi^{ks}_j\}$ are not \textit{real} electronic orbitals, instead they are a useful tool to build the \textit{real} electronic density $\dens$ and other derived physical quantities.
