...
We know that each energy levels in the well is expressed as:
...
By integrating \eqref{eq:energyDensity} over an energy interval, we obtain the number of energy levels contained in that interval:

\begin{equation}
	\Delta \xi_{\varepsilon_1 \varepsilon_2} = \int_{\varepsilon_1}^{\varepsilon_2} g(\varepsilon) \dd{\varepsilon}
\end{equation}

To calculate to total energy of the box (which is our final goal), we need to know how many levels are occupied by electrons. 
More precisely, we need the probability that a given energy level is occupied by an electron, namely the partition function of the system. 
...
\begin{equation}
	f(\varepsilon) = \left\lbrace \begin{array}{ll}
		1 & \mbox{if $\varepsilon \leq \varepsilon_{f} $}  \\
...
As a constraint we have that the integration of the electronic density must yield the total number of electrons.
\begin{equation}\label{eq:TFDensityConstraint}
	N = \int \rho(\mathbf{r}) \dd{\mathbf{r}} 
\end{equation}
Then the equation for Lagrange multipliers become:
\begin{equation}
...
\begin{equation}
	\phi(\mathbf{r}) = \frac{Z}{r} - \int \frac{\rho(\mathbf{r_2})}{\mid \mathbf{r} - \mathbf{r_2} \mid}
\end{equation}
and write :
\begin{equation}
...
By solving equation \eqref{eq:TFEq} and \eqref{eq:TFDensityConstraint} the final $\rho(\mathbf{r})$ and $\mu_{tf}$ are obtained.

As in the case of the pure Hartree method we are simplifying the model by neglecting any exchange and correlation effect. 
The correction to this approximation was made by Dirac in the Thomas-Fermi-Dirac atomic model.



\subsubsection{Thomas-Fermi-Dirac Atomic Model}

The contribute of P.A.M. Dirac was to model the exchange term \footnote{correlation effects are still neglected by the system} in the following way.\footnote{We are considering only closed-shell systems.}
...
This non-local nature of \eqref{eq:excDensity} poses a great challenge in the solution of equation \eqref{eq:TFEq}.
By using plane waves as spatial orbitals \footnote{remeber that the single particle is contained in a box} and introducing the condition that the solution must be periodic in $l$ and it must be null on the edges of the box, we can re-express \eqref{eq:diracExc} as :

\begin{align}
	& K[\rho] = C_{x} \int \rho^{\frac{4}{3}}(\mathbf{r}) \dd{\mathbf{r}} & C_{X} = \frac{3}{4} \left(\frac{3}{\pi} \right)^{\frac{1}{3}}
\end{align}


This process is an example of \textit{local density approximation} or LDA, consisting in the reduction of the dependency of $K$ only to a single spatial coordinate.
...
Unfortunately the results obtained by this model are not satisfying, but the TFD model is the springboard for DFT development.
