...
\texttt{fft\_scatter} time seems rather constant but his value compared to the one of \texttt{fftw} increases with the number of core used. 
In fact, as plotted in the graph on the right, we can see that CPU time ratio of \texttt{fft\_scatter} over \texttt{fftw} rises over 16 cores, when more than two Galileo nodes, and more than four NUMA nodes on two different blades, have to communicate.
