...
Since, thanks to the Bloch's theorem, the calculation for each $\mf{k}$ point is independent and, as said above, we focused on system in gamma-point configuration, we will consider $\mf{k}=0$ in order to simplify the exposition without loosing important information regarding the implementation.
Consequently any following consideration is valid within a given (and fixed) k-point sub-block of $\mf{H}^{KS}$.

From equation \eqref{eq:BlochEq} we have clear that we need a set of informations to perform the calculation :
\begin{enumerate}
	\item We need an expression of $v_{eff} = v + v_{h} + v_{xc}$;
	\item We need to apply efficiently the operator $\nabla^2$ to any $u_{\mf{k}j}$;
	\item We need to perform the diagonalization of $\mf{H}^{KS}$;
\end{enumerate}

\subsubsection{Evaluating the potential}
...
\begin{equation}
	\tilde{v}_{\alpha}(\mf{G}) = \sum_{\mid\mf{G}\mid < G_{cut}} f_{ps}^{sp}(G) e^{i \mf{G} \cdot \mf{R}_{\alpha}},
\end{equation}
where $G_{cut}$ is the maximum module of a reciprocal vector below the cutoff energy \eqref{eq:cutoffEnergy}, $\mf{R}_{\alpha}$ is the position of the ion and $f_{sp}^{sp}(G)$ is the function describing the pseudopotential of the given atomic species ``\textit{sp}" (e.g. hydrogen, oxygen, sodium, ...).
...
We know that any electrostatic potential must satisfy the Poisson's equation :
...
The computational advantage of \eqref{eq:HartreePotReciprocal} over \eqref{eq:HartreePot} is that we do not need to perform the integration over the real grid composing the elementary cell, the complexity lies in the evaluation of $\tilde{\rho}(\mf{G})$ which is computed at the beginning of each SCF step along with $\dens$.

\mynotes{di questa parte sulla complessita' non sono sicuro:}

Neglecting the constants we have that 
\begin{equation}
	\mathcal{O}(\tilde{v}_{h}) = 	\mathcal{O}(\tilde{\rho}) = \mathcal{O}(\numbands)
\end{equation}
instead of 
\begin{equation}
	\mathcal{O}(v) = \mathcal{O}(\numbands \cdot \numG)
\end{equation}

because the number of points of the direct grid used to perform the integration is equal to the number of $G$ points in the reciprocal space.


Note that instead of $v(\mf{G})$, $v_{h}(\mf{G})$ is calculated once for every SCF step in the routine.
...
Then the impact on computational complexity of $v_{xc}(\erre)$ is negligible, analogously to $\tilde{v}(\mf{G})$.





\subsection{Domande per davice}
\begin{enumerate}
	\item $\tilde{\rho}$ e' calcolata come una FFT di $\dens$ oppure visto che la davidson lavora in spazio reciproco e alla fine della diagonalizzazione si ottengono gli orbitali in spazio reciproco e' il contrario, cioe' $\dens = IFFT\{\tilde{\rho}\}$ ?
	\item la complessita' della FFT va con il numero dei coefficienti, quindi con il numero di punti G? cioe' $\mathcal{O}(\numG \log \numG)$?
