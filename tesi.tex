...
Sample Output: 
\lstinputlisting[basicstyle=\small ,breaklines=true,tabsize=2]{media/listings/cpumap.out}
...
\lstinputlisting[basicstyle=\small ,breaklines=true,tabsize=2]{media/listings/numactl.out}
...
Output:
\lstinputlisting[basicstyle=\small ,breaklines=true,tabsize=2]{media/listings/topology.out}
...
Unfortunately it is not possible to select the exact CPUs to occupy, thus we cannot see if reserving the first eight CPUs\footnote{Being on the same CPU-set, they share the same L3 cache.} would lead to the same result than shared nodes.
...
\footnote{\texttt{taskset} and \texttt{dplace} are two other tools to provide process pinning, but they have proven to be not reliable during some preliminary tests.}.
