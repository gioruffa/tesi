...
\centerline{ \includegraphics[width=1.2\linewidth]{relevant_subroutines.pdf}	}
	\caption{CPU time (left) and speedup (right) for relevant subroutines listed in section \ref{sec:resCriticalComponents}. Data from runs at fixed number of SCF steps.}
	\label{fig:relevantSubroutines}
\end{figure}

As said in section \ref{sec:resCriticalComponents}), \texttt{cegterg} has the same behavior of PWscf, we can use it a reference for PWscf execution time.
...
It is clear that the scalability problem must lies withing \texttt{cdiaghg}.
...
In general it is suggested that to total number of threads must not exceed the number of physical cores the resource has (see section XXX).
...
From the data exposed in \ref{app:Threads} we have composed two optimal set of runs for fixed and variable iterations\footnote{The difference is only at 32 cores.} number of iterations.
The ratio was to keep the best result per total number of cores employed, that is number of MPI ranks times number of threads per rank.

The results, compared to the runs without threads, are exposed in figure \ref{fig:threadsOverall}.
...
The spike at eight cores is completely disappeared and the descending trend of the CPU time is much more regular, hence efficient.
...
We have annotated when distributed diagonalization starts on MPI only runs and when serial diagonalization ends on threaded runs.
...
Note that the gain obtained from eight to sixteen cores cannot be ascribed to the effect of threads because the number of threads used is the same and the algorithm is serial.
...
\centerline{ \includegraphics[width=\linewidth]{threads_subroutines.pdf}	}
	\caption{Speedup of relevant subroutines at fixed number of iterations for threaded and not threaded PWscf's runs.}
	\label{fig:threadsSubroutines}

\end{figure}


The plot of \texttt{cdiaghg}'s speedup in figure \ref{fig:threadsSubroutines} tells us that threads mainly benefits diagonalization\footnote{The fact that speedup does not change from eight to sixteen cores confirm what we have said above.}.
...
To optimize the number of simulations completed one could perform a set of tests on a trial system and determine the right number of threads to obtain a reasonable efficiency.
...
This is because all these benchmarks focuses on large scale computing, typically starting at least from 64 cores, up to thousands of cores.
Although these data are of great interest, not all the scientific community operates on such a large scale.

One must also consider that a great optimization effort was dedicated to increase the performance of the evaluation of $\mf{H}^{KS}$, which translates in a more efficient FFT calculation (see section \ref{sec:QE}). 
