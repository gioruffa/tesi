...
\mynotes{	metti la roba con la graffa lunga e gli arrai che e' pesante da scrivere}
...
Note that expression \eqref{eq:excDensity} do not depends only on a single spatial coordinate, but on two of them.
This non-local nature of \eqref{eq:excDensity} poses a great challenge in the solution of equation \eqref{eq:TFEq}.
The reduction of the dependency of $K$ only to a single spatial coordinate is called a \textit{local density approximation} or LDA.
...
The model is the springboard for DFT
