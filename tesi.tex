...
Making a rough estimate, the diagonalization of the dense matrix would require $N_{G}^{PW} \cdot N_{G}^{PW} \simeq N^2$ elements to be evaluated and stored in memory; Davidson diagonalization only requires $N_{b}^2$ elements.
...
the trial eigenvectors $\{\ket{u_{kj}}\}^{in}$  are the ones obtained in the previous SCF step, if any, or the ones obtained in \texttt{init\_run}.
...
As a matter of fact $\densin$ could not be generated from $\{\ket{u_{kj}}\}^{in}$; 
...
As a matter of fact $\{\ket{u_{kj}}\}^{in}$ are a reasonable guess of the new eigenstates $\{\ket{u_{kj}}\}^{out}$, reducing the number of Davidson steps required to reach convergence.
}
...
\paragraph{Parallel Diagonalization: } In this case \QE would leverage the features of the SCALAPACK library and distribuite the load over a set of MPI ranks. 
...
The IFFT transform constitute the major computational cost, and it's comparable with the cost of a single Davidson iteration.
...
What happens in modern solid state simulations is that DFT and HF are used together to simulate the same system. The final contribution in than weighted as $75\%$ DFT and $25\%$ HF. Both methods sharing the same plane wave basis set.
