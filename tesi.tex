...
qui parla di come da formule di marx viene fatta l'implementazione di FFT.
Praticamente si parallelizza il calcolo delle colonnette su z e poi si fa una all to all su tutti i piani, a loro volta si parallelizzano i piani.
Il conto del marx non e' indipendente per dimensione ma e' fattorizzabile!!! Vuol dire che puoi nestare i cicli!!!

\mynotes{Overloadare i thread non conviene perche nella fft non ci sono race conditions! Sono totalmente indipendenti i threads}
...
\end{equation}
Then consider the terms which are quadratical in $\Delta\dens$ in the quantity $E[\dens + \Delta\dens] - E[\dens ]$.

One obtains
\begin{align}
	E[\dens + \Delta\dens] - E[\dens ] \simeq \frac{e^2}{2} \iint \frac{\Delta\rho(\erre) \Delta\rho(\erre')}{\mid \erre - \erre' \mid} \dd{\erre} \dd{\erre'}\\
...
cerchi il termine quadratico nella densita' in \eqref{eq:DFTEnergy}, alla fine ti viene fuori che il termine ione e' lineare (quindi se ne va), il termine in autovalori e' costante, il termine in hartree resta solo in parte ed e' quello che si usa perche' la derivata funzionale seconda del potenziale di scambio e' troppo costosa da applicare.
