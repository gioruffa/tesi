...
%<<<<< unica parte palesemente copiata da Atkins
...
The calculation is greatly simplified if we consider the system to be at 0K. In this case $\beta = (k_B T)^{-1} \rightarrow + \infty$ and the Fermi-Dirac distribution \eqref{eq:fermiDiracPartitionFunction} become:
...
Then equation \eqref{eq:TFEq} become:
\begin{equation}
...
\subsubsection{Kohn and Shan Equations}

Kohn and Shan \cite{KS} developed a practical method to solve equation \eqref{eq:FoundamentalDFT}.
...
Substituting \eqref{eq:KSExchangeEnergy} in \eqref{eq:KSEnergy}, equation \eqref{eq:FoundamentalDFT} become 
\begin{equation}\label{eq:KSChemPot} 
	\mu_{KS} = \frac{\var{T_{ref}[\dens]}}{\var{\dens}}  + v_{eff}(\erre),
\end{equation}
with 
\begin{align}
 v_{eff}(\erre) &= v(\erre)  + v_{H}
 + v_{xc}(\erre) \\
 v_{h}(\erre) &= \frac{1}{2} \int \frac{\rho(\mathbf{r'})}{\mid \mathbf{r} - \mathbf{r'} \mid}  \dd{\mathbf{r'}}\\
...
\boxed{
	\lbrace h_{1} + \frac{1}{2} \int \frac{ \rho(\mathbf{r'}) } {\mid \erre - \mathbf{r'}  \mid} \dd{\mathbf{r'}}  + v_{xc}(\erre) \rbrace 
		\psi_{j}^{KS}(\erre) 
...
Equations \eqref{eq:KSeq} are called the \textit{Kone-Shan equations}, once solved it's possible to calculate the electronic density using definition \eqref{eq:densityDef}.
...
\begin{equation}\label{eq:DFTMatrix}
	\mathbf{H} \mathbf{C} = \mathbf{S C} \varepsilon,
\end{equation}

where 
\begin{equation}\label{eq:DFTH}
	\mathbf{H}_{ij} = \int \psi^{*}_{i}(\erre) \lbrace h_{1} + \int \frac{ \rho(\mathbf{r'}) } {\mid \erre - \mathbf{r'}  \mid} \dd{\mathbf{r'}}  + v_{xc}(\erre) \rbrace \psi_{j}(\erre) \dd{\erre},
\end{equation}
$\mathbf{C}$ is matrix of the expansion coefficients \eqref{eq:coeffMatrix} and $\mathbf{S}$ is the overlap matrix.

We can see that from a computational standpoint there is a deep difference between the $\mathbf{H}$ matrix and the Fock matrix $\mathbf{F}$: evaluation of $\mathbf{H}_{ij}$ is much easier than $\mathbf{F}_{ij}$ because there is no need to evaluate the onerous exchange integrals $(\mu \lambda | \sigma \nu)$. Al the exchange-correlation contribution is reduced to a multiplicative potential $v_{xc}(\erre)$

In fact the complexity associated to DFT calculation is lower than a standard HF calculation, but it depends strongly on the choice of basis set. Please refer to section \ref{sec:QE} for further details.

Before proceeding further it is convenient to reformulate the total energy functional $E[\dens]$.

By defining 
\begin{equation}
	V = \int \dens v(\erre) \dd{\erre}
\end{equation}
and substituting \eqref{eq:HartreeEnergy} in equation \eqref{eq:KSEnergy}, the energy functional become:
...
Although the Thomas-Fermi model can seem a good approximation we can use equations \eqref{eq:KSeq} in our favor.
For a diagonal set of $\{\psi^{KS}_{j}\}$\footnote{Guaranteed to exist after the diagonalization of \eqref{eq:DFTH}} equations \eqref{eq:DFTH} has the form :
\begin{align}
	&\sum_{j} \bra{\psi^{KS}_{j}}\mathbf{H}\ket{\psi^{KS}_{j}} = \\
...
\end{align}
Equation \eqref{eq:DFTEnergy} is the one used by the majority of \textit{ab initio} calculations softwares.
...
Because the Culombic term and $v_{xc}$ in \eqref{eq:KSeq} implicitly depend on the orbitals set trough $\dens$ (see eq. \eqref{eq:densityDef}) an interative SCF approach must be used. 

We briefly outline the general Self Consistent approach used in DFT calculations.
...
In contrast to the HF-SCF method, instead of guessing the orbitals (or their expansion coefficients), we guess the electronic density $\dens$. Usually, as a first rough approximation, the atomic electronic densities of each atom are used.\footnote{Note that any initial guess is equally valid, but a more reasonable one will make the iteration converge faster.}

The SCF iteration can now begin. 
We will call the initial density $\rho^{in}(\erre)$ and $\rho^{out}(\erre)$ will be the density obtained at the end of each iteration step.
...
\begin{enumerate}
	...
	\item self consistency is tested. A wide range of methods can be used depending on the implementation. Usually the value $\int \mid \rho^{out}(\erre) - \rho^{in}(\erre)\mid \dd{\erre} $ is evaluated in conjunction with the difference between energies $E^{in}[\dens] - E^{out}[\dens]$, but more refined methods are available;
	\item if self-consistency is achieved the computation is completed and the final electronic density is the solution of equation \eqref{eq:FoundamentalDFT}. Otherwise $\rho^{out}(\erre)$ is used to determine the next $\rho^{in}(\erre)$. Usually $\rho^{out}(\erre)$ is mixed with the electronic densities obtained in the previous iterations (if any). This process guarantees a faster SCF convergence.
\end{enumerate}



As one can imagine, the main computational complexity is contained in step \ref{en:SCFDiag}.
The choice of the basis has great impact on the implementation of this step, both for the evaluation of $H \ket{\psi}$ and the diagonalization strategy. 

As a final consideration of this theoretical section we must say that from a physical sandpoin the Kone-Shan eigenvalues $\varepsilon^{KS}_j$ do not really represents energies. Also the orbitals  $\{\psi^{ks}_j\}$ are not \textit{real} electronic orbitals, instead they are a useful tool to build the \textit{real} electronic density $\dens$ or other derived physical quantities.


We shall now proceed with an in-depth analysis of the implementation of DFT SCF methods inside the Quantum ESPRESSO suite, one of the most popular and solid software for materials modeling.
