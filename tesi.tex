...
Using a more formal approach outlined in \cite[p.357-362]{Sakurai} we can express the exchange two coordinates in \eqref{eq:HartreeProduct} as the action of the transposition operator $P_{ij}$
\begin{align*}
...
For the ket state to be antisymmetric it must be an eigenfunction of $\hat{P}$ with eigenvalue $-1$.
\begin{align*}
	\hat{P_{ij}} & \ket{\chi_1(\mathbf{x}_1)   \cdots  \chi_i(\mathbf{x}_i)  \cdots  \chi_i(\mathbf{x}_j)  \cdots   \chi_N(\mathbf{x}_N)} =  \\
