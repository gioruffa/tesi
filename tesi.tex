...
Although the universality of this equation, we know very well that for not-so-complex systems the solution is not analytical. 
...
Nuclear mass is much higher than electron mass, thus we expect that the electrons will have much higher velocities than the nuclei. So we can separate the motion of the system in two distinct movements: the "slow" movement of the nuclei, and the "fast" movement of the electrons.

Another formulation is that from the point of view of electrons, the nuclei appear to be fixed. 
From a physical standpoint the electrons are moving in the static field produced by the nuclei while still interacting with each others.

\mynotes{ATKINS pg 241}

In this approximation we reduce our general Hamiltonian in the following form:

\begin{equation}
	\hat{H}_{tot} = \hat{T}_{e} + \hat{V}_{Ne} + \hat{V}_{ee}
\end{equation}

What we are neglecting is the effect the electrons have on the nuclear structure of each nucleus.
...
%\centerline{
%\hfill
%% NOTE the rotation here below:
%\includegraphics[angle=18,width=30mm,clip=]{Common_Cockroach.eps}
%% this is a way to combine a second picture horizontally:
%\hfill
%\includegraphics[angle=-18,width=30mm,clip=]{Common_Cockroach.eps}
%\hfill
%}
%\caption{\label{rot_Cockroach:fig}
%Small rotated coakroaches.
%}
%\end{figure}


%See more details in Fig.~\ref{rot_Cockroach:fig}
