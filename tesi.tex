...
The basic idea behind the TF model is to divide the space around the atom in a grid of boxes of equal length $l$, each box containing $\Delta N$ electrons (the number can vary from box to box). 
...
\mynotes{non ho capito come si ovvia al fatto che l'intergrale Coulombiano diverge per $r_1 = r_2$. E' implicito che l'integrazione va effettuata per $r_1 \neq r_2$?}
...
Our unique constraint is that integrating the electronic density we must obtain the total number of electrons.
...
The contribute of P.A.M. Dirac was to model the exchange term \footnote{Correlation effects are still neglected} in the following way.\footnote{We are considering only closed-shell systems.} \cite[p.106]{Parr}
...
By using plane waves as spatial orbitals \footnote{remeber that the single particle is contained in a box} 
\begin{align}
