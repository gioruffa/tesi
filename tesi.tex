...
In section \ref{sec:SCF-DFT} we gave a basic representation of an SCF algorithm based on DFT (see figure \ref{fig:SCF-DFT}).
...
Using the above convention $u_{jk}$ became:

\begin{align}
	u_{kj}(\erre) = \frac{1}{\sqrt{\Omega}} \sum_{\GI} c_{kj}(\GI)e^{i \GI \cdot \erre}.
\end{align}

Which coincides with the use of a Discrete Fourier Transform, being $c_kj(\GI)$ the Fourier coefficients.
...
\begin{align}
	f(\erre) &= \sum_{\GI} \tilde{f}(\GI) e^{i \GI \cdot \erre} \\
			 &= \sum_{\mf{b}} \tilde{f}(\GI) exp\left[ \frac{2\pi}{N_{x}} i b_x q_x \right] + exp\left[ \frac{2\pi}{N_{y}} i b_y q_y \right] + exp\left[ \frac{2\pi}{N_{z}} i b_z q_z \right],
\end{align}
...
From equation \eqref{eq:BlochEq} it's clear what are the key components of any DFT-SCF calculation:
\begin{enumerate}
%	\item We need the expression of $\dens$ and his Fourier transform $\rdens$;
	\item evaluate $v_{eff} = v + v_{h} + v_{xc}$;
	\item apply the operator $\nabla^2$ to any $u_{\mf{k}j}$;
	\item perform the diagonalization of $\mf{H}^{KS}$;
\end{enumerate}


In section \ref{sec:Grid} we showed that we have two valid representations of the system, one in direct space and the other in reciprocal space. We can leverage this feature to calculate the terms in $\mf{H}^{KS}$ where it is more convenient and then transpose the results using FFT transformations\footnote{This is why we preferred not to specify any dependency of $v$,$v_{h}$,...}.

This concept is the key model upon which PWscf has been developed.


\subsubsection{Evaluating the potential}\label{sec:Potential}

The potential $v_{eff}$ is composed by three terms:
\begin{equation}
	v_{eff} = v + v_{h} + v_{xc},
\end{equation}
where $v$ is the electron-ion potential, $v_{h}$ the electrostatic Hartee potential and $v_{xc}$ is the exchange-correlation potential.
...
A discussion on the nature of pseudopotentials is out of the scope of this work, however a good dissertation on the matter can be found in \cite[chap. 11]{Martin}.
...
Note that because ion position is fixed under the Born-Oppenheimer approximation, $\tilde{v}(\mf{G})$ is calculated only once for the whole SCF iteration, then its impact on the computational cost is negligible.  

The calculation of $\tilde{v}$ happens inside the \texttt{init\_run} routine, which is the first routine called by PWscf and it's called only once in the whole execution (see section \ref{sec:CoreRoutines} for further details).
...
Using the definition \eqref{eq:HartreePot} requires to compute the integral for every point of the real grid . That is : for every of the $N$ point of the real grid (labeled by $\mf{r}$) one must sum (integrate) again for every point of the real grid $N$ (labeled by $\mf{r}'$). This leads to a complexity of $\mathcal{O}(N^2)$.

\mynotes{magari lo mettiamo in appendice}
...
As specified in \cite[Appendix A.2]{QE}, the strategy adopted by \QE is to use an algorithm called the \textit{Davidson method}\cite{Davidson}\footnote{\QE has also an alternative algorithm, called the \textit{conjugate gradient} method.}.
...
That is : there is no need to store the whole matrix in memory!
...
\begin{equation}
	\mf{H^{KS}} \ket{u_{kj}}.
\end{equation}
This operation is implemented inside well-named subroutine \texttt{h\_psi}.

Inside \QE the Davidson diagonalization is performed in the reciprocal space and works as follows:
...
\begin{enumerate}
	...
	\item transform the result to the reciprocal space : \\ $[v_{eff}(\mf{R}) u_{kj}(\mf{R})](\GI) = FFT\{v_{eff}(\mf{R}) u_{kj}(\mf{R})\}$;
\end{enumerate}

This means that for every call of \texttt{h\_psi} an FFT and an IFFT must be performed, each of them has complexity $\bigO(N log (N))$. 

Since \texttt{h\_psi} is called once over all the $\{u_{kj}\}$, the total complexity is $\bigO(N_{b} N log (N))$, where $N_{b}$ is the number of bands (that is of the order of the number of electrons in the system).

\mynotes{manca la parte di algebra lineare della Davidson}
\mynotes{manca schema che e' fondamentale}
\mynotes{manca sum\_bands}
