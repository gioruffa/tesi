...
Titanium dioxide (Titania) has been the subject of extensive research in recent years due to its relevant photochemical and photophysical properties combined with its natural abundance, low cost, and superior chemical and photostability. 
...
It is important in earth sciences, plays a role in the biocompatibility of bone implants, is being discussed as a gate insulator for the new generation of MOSFETS and as a spacer material in magnetic spin-valve systems, and finds applications in nanostructured form in Li-based batteries and electrochromic devices.
...
\begin{itemize}
	\item the lower the size the same is the scaling $\rightarrow$ cluster is better
	\item the bigger the size the more important is the matrix diagonalization $\rightarrow$ NUMA is better
\end{itemize}
...
Mi piacerebbe costruire una progressione con le migliori run dei threads a basso numero di core e i migliori dati ottenuti ottimizzando la diagonalizzazione e confrontarlo con il comportamento di default dell'espresso.

E' un modo per dire che seguendo le regole che abbiamo estrapolato e' possibile effettuare un tuning su entrambe le architetture a prescindere dal numero di core che si vogliono utilizzare, senza dover mettere le mani nel codice o settare parametri esotici.

Della serie se hai fretta (e vuoi usare tanti core) fai questo (ndiag), se invece vuoi fare un sacco di simulazioni contemporanemante fai quest'altro (threads).


\section{Outlooks}

Nuovi processori vanno verso le grandi quantita' di cores, sara' interessante vedere se converra' utilizzare la diagonalizzazione seriale o quella parallela e se varra' ancora la pena perdere performance nei confronti delle FFT.
