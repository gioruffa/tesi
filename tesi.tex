...
In the following we will show the calculation needed to perform each SCF step as used in the PWscf package.

Since, thanks to the Bloch's theorem, the calculation for each $\mf{k}$ point is independent and, as said above, we focused on system in gamma-point configuration, we will consider $\mf{k}=0$ in order to simplify the exposition without loosing any important information regarding the implementation.
...
The potential $v_{eff}$ is composed by three components:
\begin{equation}
	v_{eff} = v + v_{H} + v_{xc},
\end{equation}
