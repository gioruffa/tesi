...
\mynotes{N e' davvero un pessimo numero perche' e' uguale al numero di elettroni}
\begin{equation}
	\Delta N_{\varepsilon_1 \varepsilon_2} = \int_{\varepsilon_1}^{\varepsilon_2} g(\varepsilon) \dd{\varepsilon}
\end{equation}

To calculate to total energy of the box (which is our final goal), we need to know what is the probability that a given energy level is occupied by an electron, namely the partition function of the system. Since we are dealing with fermions, the Fermi-Dirac statistic must be used:
...
As a constraint we have that the integration of the electronic density must yield to total number of electrons.
...
\mynotes{cita Parr, dovrebbe essere pag 246-254}
\begin{align}
