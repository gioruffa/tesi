...
This means that in the continuous-indexed energy matrix of eq \eqref{eq:SCHFourier} most off-diagonal elements vanishes because to have a non-null element the following relation must stand $\mf{k} = \mf{k'} + \mf{G}$.

Then the general matrix became a diagonal block matrix, meaning that one can diagonalize only the blocks on non-null blocks on the diagonal, and, more importantly, the operation can be performed independently for each block (i.e. in parallel!).



figate onde piane : problema continuo e' diventato discreto, la matrice e' diagonale a blocchi, la derivazione e' una moltiplicazione in spazio reciproco!
