...
Because the Culombic term in \eqref{eq:KSeq} implicitly depends on the orbitals set trough $\dens$ (see eq. \eqref{eq:densityDef}) an interative SCF approach must be used. 


\mynotes{SEI QUI !}

One $\dens$ is obtained, the final evaluation of $E[\dens]$ follows from the equation
\begin{equation}
...
One must decide in which form the Kinetic energy $T_{ref}[\dens]$ is expressed. Usually the electrons are modeled as a gas of independent fermions, exactly as done in the TF model, then the kinetic functional takes the form expressed in \eqref{eq:TFKinetic}.
...
\mynotes{Mettero' una spiegazione del metodo SCF usato nella DFT con le onde piane nella parte di espresso, dove tratto anche la complessita'.Non mi sembra il caso di metterla qui.}
