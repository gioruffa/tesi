...
\texttt{Fft\_scatter} CPU time is subject to little variation, but if we compare its value with \texttt{fftw}'s one we can see that communication gradually earns an increasing weight. 
In fact, as plotted in the graph on the right, we can see that CPU time ratio of \texttt{fft\_scatter} over \texttt{fftw} rises over sixteen cores, when more than two Galileo nodes, and more than four NUMA nodes on two different blades, have to communicate.
...
We can see that the iteration time is far from being constant and varies sensibly. In the plot on the right we have the same data plotted in logarithmic scale to highlight the fact that that the relative variation of iteration time grows with the number of cores.
...
reasonable run time\footnote{The time we had at disposal for profiling was limited and the number of tries performed was considerable, hence the need to contain the total run time.}.
...
An alternative approach, which is mainly used in industry benchmarks, is to fix the WALL time dedicated to computation and count the number of iterations performed.
...
So does \texttt{sum\_band}, as expected for what we said in section \ref{sec:resCriticalComponents}.
...
In general it is suggested that the total number of threads must not exceed the number of physical cores on the resource (see section XXX).
...
From the data exposed in \ref{app:Threads} we have composed two optimal set of runs for fixed and variable \footnote{The difference is only at 32 cores.} number of iterations.
The \textit{ratio} was to keep the best result per total number of cores employed.\footnote{ The total number of cores is the number of MPI ranks times the number of threads per rank.}
...
In fact, by looking at to plot at fixed number of iterations on the right of figure \ref{fig:threadsOverall}, we see that, especially at eight core the advantage is still substantial.
...
From the data reported in figure \ref{fig:threadsOverall} and appendix \ref{app:Threads} we can extract a simple ``rule of thumb" to find the best number of cores:
it seems reasonable to fill the chipset with the maximum number of threads, that is to use a single MPI rank for chipset and as many threads as the cores on the chipset.
...
To begin we analyze the mean iteration time of PWscf on both architectures in order to have a general idea (see section \ref{sec:resCriticalComponents} to understand the validity of mean iteration time study).
...
This is true,but note that the x axis is not on a linear scale, more on a logarithmic scale (in base 2)\footnote{Given the presence of error bars, this kind of display was favored since it does not overcrowd the left part of the plot.}.
...
While \texttt{c\_diag} mantains a low speedup, at least on the UV2000 the performance rises above the sixteen cores.
...
Finally, the plot of \texttt{fft\_scatter}\footnote{We remind that \texttt{fft\_scatter} is a subroutine that performs manily communication.} strongly suggests that the difference in performances can be attributed to the difference in communication performance.


It seems time to investigate at a deeper level the role of communication in PWscf calculations.
...
We still have one way on how diagonalization is performed, but this way is effective only on a sufficiently high number of cores.
...
We can see that the behavior of the two architecture is different.

Starting from Galileo we can clearly see that increasing $n_{diag}$ is very convenient when one cluster node is used, i.e. at sixteen cores.

On the contrary it is convenient to reduce the number of cores to sixteen when two or more cluster nodes are used.
...
An this is true regardless of the total number of cores\footnote{It is only interesting to vary the $n_{diag}$ when 16 or more core are used since the only value possible for 8 cores is four. Remember that we have already found a good tuning approach on 8 cores in section \ref{sec:resArchDependent}}.

We can also see that the default choice of $n_{diag}$ performed by \QE is the optimal one only in the case of 32 cores on Galileo.
...
\begin{figure}[hhh!]
\centerline{ \includegraphics[width=0.7\linewidth]{concl_oled.pdf}	}
	\caption{}
	\label{fig:conclOled}
\end{figure}
...
\newpage
\subsection{Ndiag}\label{app:ndiag}
\begin{figure}[hhh!]
